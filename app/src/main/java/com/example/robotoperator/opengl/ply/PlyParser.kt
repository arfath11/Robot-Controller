package com.example.robotoperator.opengl.ply

import android.opengl.GLES20
import android.opengl.Matrix
import com.example.robotoperator.opengl.IndexedModel
import com.example.robotoperator.opengl.Light
import com.example.robotoperator.R
import com.example.robotoperator.util.Util
import java.io.*
...
class PlyParser(inputStream: InputStream) : IndexedModel() {
        ...
        for (i in 0 until vertexCount) {
            ...
            if (bigEndian) {
                if (areVerticesDouble) {
                    x = Double.longBitsToDouble(Util.readLongBe(tempBytes, xOffset)).toFloat()
                    y = Double.longBitsToDouble(Util.readLongBe(tempBytes, yOffset)).toFloat()
                    z = Double.longBitsToDouble(Util.readLongBe(tempBytes, zOffset)).toFloat()
                } else {
                ...
                if (areVerticesDouble) {
                    x = Double.longBitsToDouble(Util.readLongLe(tempBytes, xOffset)).toFloat()
                    y = Double.longBitsToDouble(Util.readLongLe(tempBytes, yOffset)).toFloat()
                    z = Double.longBitsToDouble(Util.readLongLe(tempBytes, zOffset)).toFloat()
                } else {
